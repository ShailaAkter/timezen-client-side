...
const PremiumCollection = () => 
{
    ...
    const [loadMoreProducts, setLoadMoreProducts] = useState(3);

    const showMoreProducts = () =>
    {
        setLoadMoreProducts((prevProducts) => prevProducts + 3);
    }
  ...
  return (
    <div className="parallax-container container mx-auto my-8 py-20">
        <div 
        data-aos = "fade-down"
        className="text-center mb-8 py-5">
            <h1 className="text-3xl font-bold text-slate-800">Our Premium Collection</h1>
            <p className="text-sm text-slate-700 -mt-3 italic">Browse our premium collection with the best services and find a watch that fits your attitude</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8 px-10">
        {
            topProducts.slice(0, loadMoreProducts).map((product) => (
                <div 
                data-aos = "fade-up"
                key={product._id}
                className="shadow-lg overflow-hidden transform transition-all duration-300 rounded-lg hover:shadow-2xl hover:-translate-y-2 bg-slate-50 py-5">
                    <img
